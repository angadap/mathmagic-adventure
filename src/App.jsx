...
function StudentLogin({ onBack, onDone }) {
  ...
  const [loading,    setLoading]    = useState(false);
  const [error,      setError]      = useState("");
  const handleSchool = () => { if(!schoolCode.trim()){setError("Enter school code");return;} setError(""); setStep("student"); };
  const handleLogin  = async () => {
    if(!rollNo.trim()||pin.length<4){setError("Enter username and 4-digit PIN");return;}
    setLoading(true);setError("");
    try {
      const d = await schoolApi("student_login",{school_code:schoolCode.trim().toUpperCase(),roll_no:rollNo.trim(),name:rollNo.trim(),pin,class_num:classNum?parseInt(classNum):undefined,section:section||undefined});
      if(d.student){SFX.select();onDone(d.student);}
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Baloo 2','Nunito',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px",position:"relative",overflow:"hidden"}}>
      <Starfield n={30}/>
      <div style={{position:"absolute",bottom:"4%",left:"4%",fontSize:90,opacity:0.05,animation:"floatUp 4s ease-in-out infinite",pointerEvents:"none"}}>🏫</div>
      <div style={{position:"relative",zIndex:2,width:"100%",maxWidth:420}}>
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:30}}>
          <BackBtn onClick={onBack} color={C.cyan}/>
          <div>
            <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:11,color:C.cyan,letterSpacing:3}}>SCHOOL STUDENT</div>
            <div style={{fontSize:18,fontWeight:900,color:"white",marginTop:2}}>Login to your class</div>
          </div>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:24}}>
          {["School","Student"].map((s,i)=>(
            <div key={i} style={{flex:1,display:"flex",flexDirection:"column",gap:4,alignItems:"center"}}>
              <div style={{height:4,width:"100%",borderRadius:4,background:i===0&&step==="school"||i===1&&step==="student"?C.cyan:i===0&&step==="student"?C.green:"rgba(255,255,255,0.08)",transition:"background 0.3s"}}/>
              <div style={{fontSize:11,color:i===0&&step==="student"?C.green:i===0||i===1&&step==="student"?C.cyan:C.dim,fontWeight:700}}>{s}</div>
            </div>
          ))}
        </div>
        {step==="school" ? (
          <div style={loginCard(C.cyan)}>
            <div style={{textAlign:"center",marginBottom:24}}>
              <div style={{fontSize:60,marginBottom:10,animation:"floatUp 2s ease-in-out infinite"}}>🏫</div>
              <div style={{fontSize:22,fontWeight:900,color:"white"}}>Which school?</div>
              <div style={{fontSize:14,color:C.dim,marginTop:6}}>Enter your school's unique code</div>
            </div>
            <input value={schoolCode} onChange={e=>setSchoolCode(e.target.value.toUpperCase())}
              placeholder="e.g. MATH001" maxLength={20}
              style={{...loginInput(C.cyan),fontSize:22,letterSpacing:6,textAlign:"center"}}
              onKeyDown={e=>e.key==="Enter"&&handleSchool()}/>
            {error&&<div style={{color:C.red,fontSize:14,marginBottom:12,textAlign:"center",fontWeight:700}}>{error}</div>}
            <Btn color={C.cyan} onClick={handleSchool}>NEXT →</Btn>
          </div>
        ) : (
          <div style={loginCard(C.purple)}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:22,background:`${C.cyan}14`,borderRadius:14,padding:"12px 16px"}}>
              <span style={{fontSize:22}}>🏫</span>
              <div style={{flex:1}}>
                <div style={{fontSize:11,color:C.dim}}>School Code</div>
                <div style={{fontSize:16,fontWeight:900,color:C.cyan}}>{schoolCode}</div>
              </div>
              <button onClick={()=>{setStep("school");setError("");}} style={{background:"none",border:`1px solid ${C.dim}44`,borderRadius:10,padding:"5px 12px",color:C.dim,cursor:"pointer",fontSize:12,fontWeight:700}}>Change</button>
            </div>
            <div style={{fontSize:14,color:C.dim,marginBottom:6,fontWeight:700}}>Username</div>
            <input value={rollNo} onChange={e=>setRollNo(e.target.value)} placeholder="Enter your username" style={loginInput(C.purple)}/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
              <div>
                <div style={{fontSize:13,color:C.dim,marginBottom:6,fontWeight:700}}>Class</div>
                <select value={classNum} onChange={e=>setClassNum(e.target.value)}
                  style={{width:"100%",background:"rgba(255,255,255,0.06)",border:`2px solid ${C.purple}44`,borderRadius:14,padding:"13px",color:classNum?"white":C.dim,fontFamily:"'Baloo 2',sans-serif",fontSize:15,outline:"none"}}>
                  <option value="">Any</option>
                  {["Nursery","Jr KG","Sr KG","Class 1","Class 2","Class 3","Class 4","Class 5"].map((n,i)=><option key={i} value={[10,11,12,1,2,3,4,5][i]}>{n}</option>)}
                </select>
              </div>
              <div>
                <div style={{fontSize:13,color:C.dim,marginBottom:6,fontWeight:700}}>Section</div>
                <input value={section} onChange={e=>setSection(e.target.value.toUpperCase())} placeholder="A" maxLength={3}
                  style={{...loginInput(C.purple),textAlign:"center",fontSize:20,letterSpacing:4,marginBottom:0}}/>
              </div>
            </div>
            <div style={{fontSize:14,color:C.dim,marginBottom:6,fontWeight:700}}>PIN</div>
            <input value={pin} onChange={e=>setPin(e.target.value.replace(/[^0-9]/g,"").slice(0,4))}
              type="password" placeholder="••••" maxLength={4}
              style={{...loginInput(C.purple),fontSize:32,letterSpacing:14,textAlign:"center"}}
              onKeyDown={e=>e.key==="Enter"&&handleLogin()}/>
            {error&&<div style={{color:C.red,fontSize:14,marginBottom:12,textAlign:"center",fontWeight:700,background:`${C.red}14`,borderRadius:12,padding:"10px 14px"}}>{error}</div>}
            <Btn color={C.purple} loading={loading} onClick={handleLogin}>🚀 Enter Academy</Btn>
          </div>
        )}
      </div>
